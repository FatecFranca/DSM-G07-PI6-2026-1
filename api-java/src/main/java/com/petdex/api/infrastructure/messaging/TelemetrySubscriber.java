...
public class TelemetrySubscriber {
    ...
    public Boolean receiveMessage(String message) {
        try {
            ...
            logger.info("[Telemetry Sub] Menssagem recebida: {}", message);
            JsonNode mensagemNode = mapper.readTree(message);
            JsonNode typeNode = mensagemNode.get("type");

            if (typeNode == null) {
                logger.error("[Telemetry Sub] Campo type não informado na mensagem");
                throw new IllegalArgumentException("[Telemetry Sub] Campo ´type´ não informado na mensagem");
            }
            ...
            switch (type) {
                ...
                default:
                    logger.error("[Telemetry Sub] Tipo da mensagem desconhecido {}", type);
                    throw new IllegalArgumentException("Tipo de mensagem desconhecido" + type);
            }

        } catch (Exception e) {
            logger.error("[Telemetry Sub] Erro ao processar mensagem {}", e.getMessage());
